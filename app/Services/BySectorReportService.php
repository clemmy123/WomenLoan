...
class BySectorReportService
{
    ...
    public function normalizeFilters(array $input): array
    {
        ...
        $sector = is_string($input['business_sector'] ?? null)
            ? trim($input['business_sector'])
            : '';
        $allowed = $this->sectors()->pluck('name')->all();
        if ($sector === '' || ! in_array($sector, $allowed, true)) {
            $sector = null;
        }
    ...
    public function sortOptions(): array
    {
        return collect(self::SORTS)
            ->mapWithKeys(fn (string $sort) => [$sort => __('by_sector_reports.sort_'.$sort)])
            ->all();
    }
    ...
    protected function baseQuery(array $filters): Builder
    {
        ...
        if (! empty($filters['business_sector'])) {
            $query->whereHas('businessDetails', function (Builder $q) use ($filters) {
                $q->where('business_sector', $filters['business_sector']);
            });
        }
    ...
    protected function mapRow(Loan $loan): array
    {
        ...
        return [
            ...
            'sector' => $loan->businessDetails?->business_sector ?: __('common.na'),
            'disbursed' => $this->actualDisbursedAmount($loan),
            'paid' => (float) ($payment?->amount_paid ?? 0),
            'outstanding' => $this->outstandingAmount($loan, $payment),
