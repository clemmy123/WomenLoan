...
@php
    ...
    $viewer = auth()->user();
    $isScopedIct = $viewer?->hasAnyRole(['tehama_region', 'tehama_council']) ?? false;
    $canManageRoles = $viewer?->can('manage roles') && ! $isScopedIct;
    $canViewAudit = $viewer?->can('view audit logs') && ! $isScopedIct;
    $scopedZoneName = $isScopedIct
        ? ($viewer?->zoneable?->name ?? __('admin.zone_name'))
        : null;
            ...
            <div class="app-card-header">
                ...
                @if($isScopedIct && $scopedZoneName)
                    <p class="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">
                        {{ __('admin.dashboard_users_by_role_scoped_help', ['zone' => $scopedZoneName]) }}
                    </p>
                @endif
            </div>
            <div class="app-card-padded">
                ...
                @else
                    <div class="h-64">
                        <canvas
                            id="adminRolesChart"
                            aria-label="{{ $isScopedIct ? __('admin.dashboard_users_by_role_scoped') : __('admin.dashboard_users_by_role') }}"
                        ></canvas>
                    </div>
                @endif
            </div>
