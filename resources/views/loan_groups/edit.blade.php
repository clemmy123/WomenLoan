...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        <h2 class="text-xl font-bold text-slate-900">Edit Loan Group</h2>
        <p class="text-sm text-slate-500">Updating details for: {{ $loanGroup->name }}</p>
    </div>
    
    <form action="{{ route('loan-groups.update', $loanGroup) }}" method="POST" class="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Group Members</label>
            ...
            <p class="text-[10px] text-slate-400 mt-2 italic">* Checked members are currently part of this group.</p>
        </div>

        <div class="flex items-center gap-4 pt-4 border-t border-slate-100">
            <a href="{{ route('loan-groups.show', $loanGroup) }}" class="px-6 py-3 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-100 transition-all">Cancel</a>
            <button type="submit" class="flex-1 bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all">Update Group</button>
        </div>
    </form>
</div>
@endsection