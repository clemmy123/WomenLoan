...
<head>
    <meta charset="UTF-8">
    <title>{{ __('by_age_reports.title') }}</title>
    <style>
...
<body>
    @include('partials.report-pdf-letterhead', [
        'reportTitle' => __('by_age_reports.title'),
    ])
    <x-report-pdf-meta-bar>
        {{ __('by_age_reports.region') }}: {{ $regionLabel ?: __('by_age_reports.all_regions') }}
        &nbsp;|&nbsp;
        {{ __('by_age_reports.age_min') }}: {{ $filters['age_min'] ?? '—' }}
        &nbsp;|&nbsp;
    ...
    <table class="summary">
        <tr>
            <td>{{ __('by_age_reports.total_disbursed') }}</td>
            <td>{{ format_tzs($summary['total_disbursed']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_age_reports.people_financed') }}</td>
            <td>{{ number_format($summary['count']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_age_reports.group_count') }}</td>
            <td>{{ number_format($summary['group_count']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_age_reports.total_outstanding') }}</td>
            <td>{{ format_tzs($summary['total_outstanding']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_age_reports.total_paid') }}</td>
    ...
    <table class="data">
        <thead>
            <tr>
                ...
                <th class="amount">{{ __('by_age_reports.col_disbursed') }}</th>
                <th class="amount">{{ __('by_age_reports.col_outstanding') }}</th>
                <th class="amount">{{ __('by_age_reports.col_paid') }}</th>
        ...
        <tbody>
            ...
            @empty
            <tr>
                <td colspan="6">{{ __('by_age_reports.no_results') }}</td>
            </tr>
            @endforelse
        </tbody>
    </table>

    <p class="footer">{{ __('reports.pdf_fund') }} — {{ __('by_age_reports.title') }}</p>
</body>
</html>
