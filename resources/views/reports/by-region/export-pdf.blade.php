...
<body>
    ...
    <x-report-pdf-meta-bar>
        ...
        &nbsp;|&nbsp;
        {{ __('by_region_reports.period') }}: {{ __('reports.period_'.($filters['period'] ?? 'annually')) }}
        &nbsp;|&nbsp;
        {{ __('by_region_reports.date_from') }}: {{ $filters['date_from'] ?? '—' }}
        &nbsp;|&nbsp;
    ...
    <table class="summary">
        <tr>
            <td>{{ __('by_region_reports.total_disbursed') }}</td>
            <td>{{ format_tzs($summary['total_disbursed']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_region_reports.individual_count') }}</td>
            <td>{{ number_format($summary['individual_count']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_region_reports.group_count') }}</td>
            <td>{{ number_format($summary['group_count']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_region_reports.total_outstanding') }}</td>
            <td>{{ format_tzs($summary['total_outstanding']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_region_reports.total_paid') }}</td>
    ...
    <table class="data">
        <thead>
            <tr>
                ...
                <th class="amount">{{ __('by_region_reports.col_disbursed') }}</th>
                <th class="amount">{{ __('by_region_reports.col_outstanding') }}</th>
                <th class="amount">{{ __('by_region_reports.col_paid') }}</th>
        ...
        <tbody>
            ...
            @empty
            <tr>
                <td colspan="5">{{ __('by_region_reports.no_results') }}</td>
            </tr>
            @endforelse
        </tbody>
    </table>

    <p class="footer">{{ __('reports.pdf_fund') }} — {{ __('by_region_reports.title') }}</p>
</body>
</html>
