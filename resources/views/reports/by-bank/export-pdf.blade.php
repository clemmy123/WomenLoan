...
<head>
    <meta charset="UTF-8">
    <title>{{ __('by_bank_reports.title') }}</title>
    <style>
...
<body>
    @include('partials.report-pdf-letterhead', [
        'reportTitle' => __('by_bank_reports.title'),
    ])
    <x-report-pdf-meta-bar>
        {{ __('by_bank_reports.bank') }}: {{ $bankLabel ?: __('by_bank_reports.all_banks') }}
        &nbsp;|&nbsp;
        {{ __('by_bank_reports.fiscal_year') }}: {{ ($filters['fiscal_year'] ?? null) === \App\Support\FiscalYear::ALL_KEY ? __('reports.all_years') : ($filters['fiscal_year'] ?? '—') }}
        &nbsp;|&nbsp;
        {{ __('by_bank_reports.date_from') }}: {{ $filters['date_from'] ?? '—' }}
        &nbsp;|&nbsp;
        {{ __('by_bank_reports.date_to') }}: {{ $filters['date_to'] ?? '—' }}
        &nbsp;|&nbsp;
        {{ __('by_bank_reports.generated_at') }}: {{ now()->translatedFormat('d M Y H:i') }}
    </x-report-pdf-meta-bar>

    <table class="summary">
        <tr>
            <td>{{ __('by_bank_reports.total_disbursed') }}</td>
            <td>{{ format_tzs($summary['total_disbursed']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_bank_reports.people_financed') }}</td>
            <td>{{ number_format(($summary['individual_count'] ?? 0) + ($summary['group_members_count'] ?? 0)) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_bank_reports.group_count') }}</td>
            <td>{{ number_format($summary['group_count']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_bank_reports.total_outstanding') }}</td>
            <td>{{ format_tzs($summary['total_outstanding']) }}</td>
        </tr>
        <tr>
            <td>{{ __('by_bank_reports.total_paid') }}</td>
    ...
    <table class="data">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    {{ $row['name'] }}
                    <div class="sub">{{ $row['loan_type_label'] }} · {{ $row['bank'] }} · {{ $row['track_id'] }}</div>
                </td>
            ...
            @empty
            <tr>
                <td colspan="5">{{ __('by_bank_reports.no_results') }}</td>
            </tr>
            @endforelse
        </tbody>
    </table>

    <p class="footer">{{ __('reports.pdf_fund') }} — {{ __('by_bank_reports.title') }}</p>
</body>
</html>
