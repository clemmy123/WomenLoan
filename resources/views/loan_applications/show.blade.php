...
@php
    $viewer = auth()->user();
    $isApplicantViewer = $viewer?->hasRole('applicant') ?? false;
    ...
    $cdoViewOnlyMessage = $isApplicantViewer
        ? null
        : app(\App\Services\CdoLoanScopeService::class)->viewOnlyMessage($viewer, $loan);
    ...
    if ($isSimplifiedViewer && $loan->loan_type === 'group' && $loan->group) {
        $simplifiedGroupMembers = $loan->group->members->isNotEmpty()
            ? $loan->group->members->map(fn ($member) => [
                'name' => $member->full_name ?: trim(collect([$member->first_name, $member->middle_name, $member->last_name])->filter()->implode(' ')),
                'is_leader' => (bool) $member->is_group_leader,
            ])
            : $loan->group->applicants->map(fn ($member) => [
                'name' => $member->full_name ?: trim(collect([$member->first_name, $member->middle_name, $member->last_name])->filter()->implode(' ')),
                'is_leader' => false,
            ]);
    }
...
<div class="page">
    ...
    @if($cdoViewOnlyMessage)
        @include('partials.status-card', [
            'type' => 'info',
            'message' => $cdoViewOnlyMessage,
            'class' => 'mb-6',
        ])
    @endif

    <div class="grid grid-cols-1 {{ $showAside ? 'lg:grid-cols-3' : '' }} gap-6">
        <div class="{{ $showAside ? 'lg:col-span-2' : '' }} space-y-6">
            @if($isDisbursementViewer)
                <div class="app-card app-card-padded">
                    ...
                    <div class="detail-grid">
                        ...
                        @endif
                        @include('partials.detail-field', ['label' => __('loans.requested_amount_short'), 'value' => format_tzs($loan->requested_amount)])
                        @include('partials.detail-field', ['label' => __('loans.proposed_amount_short'), 'value' => $loan->proposed_amount ? format_tzs($loan->proposed_amount) : null])
                ...
                <div class="app-card app-card-padded">
                    ...
                    <ul class="space-y-2">
                        @foreach($simplifiedGroupMembers as $member)
                            <li class="flex items-center justify-between gap-3 rounded-xl bg-slate-50 dark:bg-white/5 px-3 py-2.5 text-sm">
                                <span class="font-semibold text-slate-900 dark:text-white">{{ $member['name'] }}</span>
                                @if($member['is_leader'])
                                    <span class="loan-progress-badge loan-progress-badge--current">{{ __('groups.group_leader') }}</span>
                                @endif
                            </li>
                        @endforeach
                    </ul>
                    ...
                    <div class="detail-grid">
                        ...
                        @if($loan->loan_type !== 'group')
                            @include('partials.detail-field', [
                                'label' => __('applicants.sex'),
                                'value' => match ($applicant?->sex) {
                                    'Male' => __('applicants.male'),
                                    'Female' => __('applicants.female'),
                                    default => $applicant?->sex,
                                },
                            ])
            ...
            @else
                <div class="app-card app-card-padded">
                    ...
                    <div class="detail-grid">
                        ...
                        @include('partials.detail-field', ['label' => __('loans.current_step'), 'value' => loan_workflow_step_label($loan->current_step)])
                        ...
                        @if(! $isApplicantViewer && filled($loan->comments))
                ...
                @if(! $isApplicantViewer && $loan->approvalLevels->count())
                <div class="app-card app-card-padded">
        ...
        <div class="space-y-4">
            @if($isKmViewer)
                @include('loan_applications._progress_steps', ['loan' => $loan])
            @endif
            @if($hasWorkflow)
